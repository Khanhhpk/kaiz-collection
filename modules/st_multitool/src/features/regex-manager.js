...
export function renderManageRegexLists() {
  if (!window.TavernHelper) {
    const msg = '<div class="st-multitool-empty-msg">TavernHelper Chưa tải</div>';
  ...
  const $characterCard = $('.st-multitool-manage-script-card-header[data-target="st-multitool-manage-regex-character-list"]').closest('.st-multitool-manage-script-card');
...
function loadRegexsWithCache(type) {
  ...
  const savedOrder = JSON.parse(localStorage.getItem(`st-multitool-regexes-order-${type}`) || '[]');
...
async function openRegexEditPanel(regexId, type) {
  try {
    ...
    const regexes = await window.TavernHelper.getTavernRegexes(targetOpt) || [];
    const regex = regexes.find(r => r.id === regexId);
