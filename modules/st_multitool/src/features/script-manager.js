...
export function renderManageScriptLists() {
  ...
  const $characterCard = $('.st-multitool-manage-script-card-header[data-target="st-multitool-manage-script-character-list"]').closest('.st-multitool-manage-script-card');
...
function loadScriptsWithCache(type) {
  ...
  const savedOrder = JSON.parse(localStorage.getItem(`st-multitool-scripts-order-${type}`) || '[]');
