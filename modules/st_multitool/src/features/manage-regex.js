  ...
  regexes.forEach(regex => {
    ...
    div.innerHTML = `
      <div class="st-multitool-manage-regex-info" style="display:flex !important;flex-direction:column !important;align-items:flex-start !important;gap:6px !important;flex:1 1 auto !important;min-width:0 !important;overflow:hidden !important;">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding-left:26px;flex-wrap:wrap;width:100%;min-width:0;">
          <div class="st-multitool-regex-pill" style="font-family:'JetBrains Mono',Consolas,monospace;font-size:11.5px;color:#38bdf8;background:#0b1329;border:1px solid rgba(56,189,248,0.3);padding:2px 8px;border-radius:4px;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:inline-block;" title="${escapeHtml(findPattern)}">${escapeHtml(shortPattern)}</div>
...
export function cleanupManageRegexView() {
  ...
  if ($select.length) {
    $select.val('__AUTO__');
    if (typeof updateTargetRegexInfo === 'function') updateTargetRegexInfo();
  }
