...
2. QUY TẮC BỎ QUA GIỚI HẠN (IGNORE CUT-OFFS): Việc phản hồi bị cắt đứt giữa câu do chạm giới hạn token của hệ thống là ĐƯỢC PHÉP VÀ NẰM TRONG DỰ TÍNH. Cứ viết bình thường, không lo bị cắt.
  ...
  async _updateRegex({ id, scope, script_name, find_regex, replace_string, trim_strings, placement, disabled, run_on_edit, substitute_regex, min_depth, max_depth, markdown_only, prompt_only }) {
    ...
    return {
      ok: true,
      pending: true,
      summary: `Đã đưa vào danh sách chờ cập nhật (Scope: ${foundScope.toUpperCase()} | ID: ${targetId}): "${updated.script_name || updated._old_script_name || id}"`
    };
  }
  ...
  async _saveRegexChanges() {
    ...
    return {
      ok: true,
      pending_review: true,
      summary: `${summary.length} thay đổi đang chờ xác nhận (Áp dụng/Hủy).`
    };
  }
}
